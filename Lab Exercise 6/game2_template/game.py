...
def print_room_items(room):
    ...
    """
    Items_Exist = False;
    Items_String = "There is "

    if(len(room["items"]) != 0):
        Items_Exist = True
        for Item in room["items"]:
            if( Item == room["items"][0]):
...
def execute_take(item_id):
    ...
    for item in player.current_room["items"]:
        if(item_id == item["id"]):
            if(player.Carry_Weight > item["Mass"] and player.Carry_Weight - item["Mass"] > 0):
                ItemFound = True
                player.Carry_Weight -= item["Mass"]
...
def main():
    print("Find and return all items to reception :)")
    """
    for char in EntranceString:
        print(char, end="")
        time.sleep(0.2)
    ...
    while True:
        ...
        if (player.current_room["name"] == rooms["Reception"]["name"]):
    ...
    input("You Win press any key to exit")
