...
def remove_spaces(text):
    ...
    for char_Int in range(0, len(text)):
        if(text[char_Int] != " " and WordStarted == False):
            Improved_Text += text[char_Int]
            WordStarted = True 
        elif WordStarted == True:
            Improved_Text += text[char_Int]
            

    DeleteCount = 0
    for char_Int in range(len(Improved_Text)-1, -1, -1):
        if(Improved_Text[char_Int] == " "):
            DeleteCount += 1
        else:
            break
...
def exit_leads_to(exits, direction):
    ...
    return str(rooms[exits[direction]]["name"])
...
def print_menu(exits):
    ...
    for key in exits:
        print_menu_line(str(key), exits[str(key) ])
    print("\n")
    # COMPLETE THIS PART:
...
def menu(exits):
    ...
    while IsLooping:
        print_menu(exits)
        UserDirection = normalise_input(input(""))
        if (UserDirection in exits):
            IsLooping == False
